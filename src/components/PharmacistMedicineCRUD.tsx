...
// Debug the import
console.log('Imported rwandanMedicines for pharmacist:', rwandanMedicines.length, 'medicines')
import { 
  Plus, 
  Edit, 
  Trash2, 
...
const PharmacistMedicineCRUD = () => {
  ...
  // Debug logging
  console.log('PharmacistMedicineCRUD component loaded', { user })
  ...
  useEffect(() => {
    console.log('Loading medicines data for pharmacist...')
    ...
    // Set medicines immediately to avoid loading issues
    setMedicines(baseMedicines)
    setFilteredMedicines(baseMedicines)
    setIsLoading(false)
    
    console.log('Pharmacist medicines loading completed')
  }, [])
  ...
  if (!isLoading && medicines.length === 0) {
    return (
      <div className="p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Pharmacist Medicine Management</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div className="bg-green-50 rounded-lg p-4 border border-green-200">
              ...
              <div className="space-y-2">
                {stockMovements
                  .filter(m => m.type === 'in' && m.userRole === 'pharmacist')
                  .slice(0, 5)
                  .map(movement => (
                ...
                {stockMovements.filter(m => m.type === 'in' && m.userRole === 'pharmacist').length === 0 && (
            ...
            <div className="bg-red-50 rounded-lg p-4 border border-red-200">
              ...
              <div className="space-y-2">
                {stockMovements
                  .filter(m => m.type === 'out' && m.userRole === 'pharmacist')
                  .slice(0, 5)
                  .map(movement => (
                    <div key={movement.id} className="flex items-center justify-between bg-white rounded p-2">
                      <div className="flex items-center space-x-2">
                ...
                {stockMovements.filter(m => m.type === 'out' && m.userRole === 'pharmacist').length === 0 && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-green-50 rounded-lg p-4 border border-green-200">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="text-2xl font-bold text-green-900">{medicines.length}</p>
                  <p className="text-sm text-green-700">Total Medicines</p>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="text-2xl font-bold text-purple-900">
                    {stockMovements.filter(m => m.type === 'in' && m.userRole === 'pharmacist').reduce((sum, m) => sum + m.quantity, 0)}
                  </p>
              ...
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="text-2xl font-bold text-orange-900">
                    {stockMovements.filter(m => m.type === 'out' && m.userRole === 'pharmacist').reduce((sum, m) => sum + Math.abs(m.quantity), 0)}
                  </p>
              ...
              onClick={() => {
                const reportData = {
                  ...
                  summary: {
                    ...
                    totalAdded: stockMovements.filter(m => m.type === 'in' && m.userRole === 'pharmacist').reduce((sum, m) => sum + m.quantity, 0),
                    totalRemoved: stockMovements.filter(m => m.type === 'out' && m.userRole === 'pharmacist').reduce((sum, m) => sum + Math.abs(m.quantity), 0)
                  },
                  stockMovements: stockMovements.filter(m => m.userRole === 'pharmacist'),
                  inventoryLogs: inventoryLogs.filter(m => m.userRole === 'pharmacist')
                }
                ...
                a.href = url
                a.download = `pharmacist-inventory-report-${new Date().toISOString().split('T')[0]}.json`
                ...
                addNotification({
                  ...
                  message: 'Pharmacist inventory report has been downloaded successfully',
                  type: 'success'
                })
              }}
              className="flex items-center space-x-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"
...
export default PharmacistMedicineCRUD
